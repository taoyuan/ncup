...
module.exports = async function find() {
  ...
  _.forEach(printers, (obj, groupKey) => {
    ...
    _.forEach(obj, (p, i) => {
      ...
      console.log(`├─ ${p._description} (${clc.blue(p._protocol.toUpperCase())})`);
      console.log(`${isLast ? '└': '├'}─ ${clc.red(p._uri_pretty)}`);

      // console.log((index > 0 ? "│\n" : "") + (index < length - 1 ? "├" : "├") + "─ " + printer._description + " (" + clc.blue(printer._protocol.toUpperCase()) + ")" + "\n" + (index < length - 1 ? "├" : "└") + "─ " + clc.red(printer._uri_pretty) + "");
    });
    // increase group index
    index++;
  });

  console.log("\n" + clc.green("If you want to install one of these, please type: ") + clc.red.bold("ncup install") + clc.green(" into your shell"));
};
