 ...
 *
 */
cups.discover = async function discover(simple) {
  ...
  lines.map(_.trim).filter(utils.accept).map(utils.parse).forEach(item => {
    answer[item.type] = answer[item.type] || [];
    answer[item.type].push(simple ? item : new InstallablePrinter(item.name, item));
  });
