...
cups.findDrivers = async function findDrivers(slugs, options) {
  ...
  slugs = arrify(slugs);
  const opts = {extract: el => el.makeAndModel};
  const items = _.map(slugs, slug => {
    const tokens = slug.split(' ');
    let index = tokens.length;
    while (index > 0) {
      const items = fuzzy.filter(tokens.slice(0, index--).join(' '), drivers, opts);
